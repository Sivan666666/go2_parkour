...
class DepthNoise(torch.nn.Module):
    ...
    def forward(self, depth) -> torch.Tensor:
        ...
        # [修改点] 处理匹配失败的像素 (Invalid Disparity)
        # 匹配失败的像素现在的 disparity 是 invalid_disp (1e7)，算出的 depth 接近 0
        # 我们在这里识别它们，并强制赋值为 max_depth
        invalid_mask = (filtered_disparity >= self.invalid_disp)
